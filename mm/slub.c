		...
		if (!slab_free_hook(s, object)) {
			...
			*head = object;
			...
			if (slab_want_init_on_free(s) && s->ctor) {
				/* Objects that are put into quarantine by KASAN
				 * will still undergo free_consistency_checks()
				 * and thus need to show a valid freepointer to
				 * check_object().
				 *
				 * Note that doing this for all caches (not just
				 * ctor ones, which have s->offset != NULL))
				 * causes a GPF, due to KASAN poisoning and the
				 * way set_freepointer() eventually dereferences
				 * the freepointer.
				 */
				set_freepointer(s, object, NULL);
			}
